...
def load_incidents() -> list[dict[str, Any]]:
    ...
    Returns:
        list: Incidents list, empty list if file is invalid.
        
    Logs:
        info: Number of incidents loaded
        error: JSON decode or IO errors (with traceback)
    """
...
def save_incidents(incidents: list[dict[str, Any]]) -> None:
    ...
    Raises:
        IOError: If file write operation fails (not caught, propagates).
        
    Logs:
        info: Number of incidents saved
        error: IO errors (with traceback)
    """
    try:
